...
public class eis_MagicVectorThruster implements EveryFrameWeaponEffectPlugin {
    ...
    private Vector2f size = new Vector2f(18.0F, 135.0F);
    ...
    private void thrust(WeaponAPI weapon, float angle, float thrust, float smooth) {
        ...
        if (this.EMGINES.getFlameColorShifter().getCurr() != weapon.getShip().getEngineController().getFlameColorShifter().getBase()) {
            weapon.getSprite().setColor(this.EMGINES.getFlameColorShifter().getCurr());
        }
